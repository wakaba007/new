...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				...
				'merlin_url'           => 'bissful-wizard',
				'parent_slug'          => 'bissful-admin-menu',
	...
	public function admin_page() {
		...
		wp_add_inline_style( 'merlin', '#qm-icon-container,#query-monitor-main{display:none}' ); // Hide Query Monitor plugin.
		 ...
		 wp_enqueue_style( 'merlin-dashboard-style',BISSFUL_URL . '/assets/css/merlin-dashboard.min.css' );
		 
		 wp_enqueue_style( 'jquery-confirm-css',BISSFUL_URL . '/assets/css/jquery-confirm.min.css' );
		...
		wp_enqueue_script('jquery-confirm',BISSFUL_URL . '/assets/js/jquery-confirm.min.js', array('jquery'), '5.0', true);
		
		wp_enqueue_script('jquery-custom-select',BISSFUL_URL . '/assets/js/jquery.customselect.min.js', array('jquery'), '5.0', true);
...
/**
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'bissful' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition merlin__content--transition-full-wdth">
			...
			<div class="demo-import-required-options">
                ...
                <td>
                   <?php
					...
					$tip = bissful__wp_kses( sprintf(  __( '<br><small>See <a href="%1$s" target="_blank">increasing memory allocated to PHP</a> or contact your hosting provider.</small>', 'bissful' ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) );

					if ( $memory < 67108864 ) {
						echo bissful__wp_kses(
							sprintf( '<code class="ssFlag ssFlag--danger">%s</code> %s %s',
								size_format( $memory ),
								__( 'Minimum value is <strong>64 MB</strong>. <strong>128 MB</strong> is recommended. <strong>256 MB</strong> or more may be required if lots of plugins are in use and/or you want to install the London Demo.', 'bissful' ),
								$tip
							)
						);
						...
						echo bissful__wp_kses(
							sprintf( '<code class="ssFlag ssFlag--warning">%s</code> %s %s',
								size_format( $memory ),
								__( 'Current memory limit is sufficient for most tasks. However, recommended value is <strong>128 MB</strong>. <strong>256 MB</strong> or more may be required if lots of plugins are in use and/or you want to install the London Demo.', 'bissful' ),
								$tip
							)
						);
						...
						echo bissful__wp_kses(
							sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s %s',
								size_format( $memory ),
								__( 'Current memory limit is sufficient. However, <strong>256 MB</strong> or more may be required if lots of plugins are in use and/or you want to install the London Demo.', 'bissful' ),
								$tip
							)
						);
					} else {
						echo bissful__wp_kses(
							sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s',
								size_format( $memory ),
								__( 'Current memory limit is sufficient.', 'bissful' )
							)
						);
					}
					?> 
                ...
                <tr>
					...
					if ( $max_input_vars < 1000 ) {
						echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--danger">%s</code> %s',
							$max_input_vars,
							__( 'Minimum value is <strong>1000</strong>. <strong>2000</strong> is recommended. <strong>3000</strong> or more may be required if lots of plugins are in use and/or you have a large amount of menu items.', 'bissful' ) )
						);

					} elseif ( $max_input_vars < 2000 ) {
						echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--warning">%s</code> %s',
							$max_input_vars,
							__( 'Current limit is sufficient for most tasks. <strong>2000</strong> is recommended. <strong>3000</strong> or more may be required if lots of plugins are in use and/or you have a large amount of menu items.', 'bissful' ) )
						);
					} elseif ( $max_input_vars < 3000 ) {
						echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s',
							$max_input_vars,
							__( 'Current limit is sufficient. However, up to <strong>3000</strong> or more may be required if lots of plugins are in use and/or you have a large amount of menu items.', 'bissful' ) )
						);
					} else {
						echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s',
							$max_input_vars,
							__( 'Current limit is sufficient.', 'bissful' ) )
						);
					}
					?>
					</td>
				...
				<?php if ( function_exists( 'ini_get' ) ) : ?>
					<tr>
						<td><?php esc_html_e( 'PHP Time Limit:', 'bissful' ); ?></td>
						<td>
							<?php
							$time_limit = ini_get( 'max_execution_time' );

							// translators: %1$s - wp codex article url.
							$tip = bissful__wp_kses( sprintf( __( '<br><small>See <a href="%1$s" target="_blank">increasing max PHP execution time</a> or contact your hosting provider.</small>', 'bissful' ), 'http://codex.wordpress.org/Common_WordPress_Errors#Maximum_execution_time_exceeded' ) );

							if ( 1000 > $time_limit && 0 != $time_limit ) {
								echo bissful__wp_kses(
									sprintf( '<code class="ssFlag ssFlag--danger">%s</code> %s %s',
										$time_limit,
										__( 'Minimum value is <strong>1000</strong>. <strong>2000</strong> is recommended.', 'bissful' ),
										$tip
									)
								);
							} elseif ( (2000 > $time_limit && 1000 <= $time_limit) && 0 != $time_limit ) {
								echo bissful__wp_kses(
									sprintf( '<code class="ssFlag ssFlag--warning">%s</code> %s %s',
										$time_limit,
										__( 'Current time limit is sufficient, however <strong>2000</strong> is recommended.', 'bissful' ),
										$tip
									)
								);
							} elseif ( 2000 <= $time_limit && 0 != $time_limit ) {
								echo bissful__wp_kses(
									sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s %s',
										$time_limit,
										__( 'Current time limit should be sufficient.', 'bissful' ),
										$tip
									)
								);
							} else {
								echo bissful__wp_kses(
									sprintf( '<code class="ssFlag ssFlag--success">%s</code> %s',
										_x( 'unlimited', 'Time limit status.', 'bissful' ),
										__( 'Current time limit is sufficient.', 'bissful' )
									)
								);
							}
							?>
				...
				<?php endif; ?>
					        ...
					        <tr>
					<td><?php esc_html_e( 'File System Accessible:', 'bissful' ); ?></td>
					<td>
						<?php
						global $wp_filesystem;

						if ( $wp_filesystem || WP_Filesystem() ) {
							echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--success">%s</code>', esc_html__( 'Yes', 'bissful' ) ) );
						} else {
							echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--danger">%s</code><p>%s</p>',
								__( 'No', 'bissful' ),
								__( 'Theme has no direct access to the file system. Therefore plugins and pre-made websites installation is not possible to work properly.<br>Please try to insert the following code: <code>define( "FS_METHOD", "direct" );</code><br>before <code>/* That\'s all, stop editing! Happy blogging. */</code> in <code>wp-config.php</code>.', 'bissful' ) )
							);
						}
						?>
					</td>
				...
				<tr>
					<td><?php esc_html_e( 'Uploads Folder Writable:', 'bissful' ); ?></td>
					<td>
					<?php
						...
						if ( wp_is_writable( trailingslashit( $wp_uploads['basedir'] ) ) ) {
							...
							echo bissful__wp_kses( sprintf( '<code class="ssFlag ssFlag--danger">%s</code><p>%s</p>',
								__( 'No', 'bissful' ),
								__( 'Uploads folder must be writable to allow WordPress function properly.<br>See <a href="https://codex.wordpress.org/Changing_File_Permissions" target="_blank">changing file permissions</a> or contact your hosting provider.', 'bissful' ) )
							);
						}
					?>
					</td>
			...
			<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
			<?php } else { ?>
			<h4 class="dh-plugin-required">Please Install and Activate Teconce Core Plugin to continue import demo.</h4>
			<?php } ?>
...
/**
  ...
  /**
	...
	function elementor_update_options() {
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'product_template', 'footer_builder', 'edd_archive_builder', 'bissful_block' ] );
	}

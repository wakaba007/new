...
jQuery( function( $ ) {
	...
	var TeconceVariationSwatchesMeta = {
		init: function() {
			$( '.teconce-variation-swatches__field-color input' ).wpColorPicker();

			$( document.body )
				.on( 'click', '.teconce-variation-swatches__field-image .button-add-image, .teconce-variation-swatches__field-image img', this.uploadImage )
				.on( 'click', '.teconce-variation-swatches__field-image .button-remove-image', this.removeImage );

			$( '#teconce_variation_swatches_data' )
				.on( 'change', '.teconce-variaton-swatches__type-field select', this.toggleSwatchesMetabox )
				.on( 'change', '.teconce-variaton-swatches__size-field select', this.toggleCustomSizeFields );
			...
			$( '#teconce-variation-swatches-new-term-dialog' )
		...
		uploadImage: function( event ) {
			...
			mediaFrame.on( 'select', function () {
				...
				$button.closest( '.teconce-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( attachment.id );
				$button.siblings( '.button-remove-image' ).removeClass( 'hidden' );

				var attachment_image = attachment.sizes && attachment.sizes.thumbnail ? attachment.sizes.thumbnail.url : attachment.url;
				$button.closest( '.teconce-variation-swatches__field-image' ).find( 'img' ).attr( 'src', attachment_image );
			} );
		...
		removeImage: function( event ) {
			...
			$button.closest( '.teconce-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( '' );
			$button.closest( '.teconce-variation-swatches__field-image' ).find( 'img' ).attr( 'src', function() {
		...
		reloadSwatchesPanel: function() {
			...
			$( '#teconce_variation_swatches_data' ).load( this_page + ' #teconce_variation_swatches_data_inner', function() {
				setTimeout( function() {
					$( '.teconce-variation-swatches__field-color input' ).wpColorPicker();
				} );
			} );
		...
		closeDialog: function( event ) {
			...
			$( '#teconce-variation-swatches-new-term-dialog' ).hide();
		},
		...
		openNewAttributeDialog: function( event ) {
			...
			var $button = $( this ),
				$dialog = $( '#teconce-variation-swatches-new-term-dialog' ),
				data = {
			...
			$( '.form-field__swatches', $dialog ).children().hide().filter( '.teconce-variation-swatches__field-' + data.type ).show();
		...
		ajaxAddTerm: function( event ) {
			...
			var $button = $( this ),
				...
				$message = $( '.teconce-variation-swatches-modal__message', $dialog ),
			...
			wp.ajax.send( 'teconce_variation_swatches_add_term', {
				data   : data,
				error  : function ( res ) {
	...
	TeconceVariationSwatchesMeta.init();
} );
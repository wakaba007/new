...
class Settings {
	const OPTION_NAME = 'teconce_variaiton_swatches';
	 ...
	 * @static
	 *
	 * @return Teconce\VariationSwatches\Admin\Settings An instance of the class.
	 */
	...
	public function get_shape_options() {
		$options = apply_filters( 'teconce_variation_swatches_shape_options', [
			'round'   => esc_html__( 'Round', 'teconce-variation-swatches' ),
			'rounded' => esc_html__( 'Rounded', 'teconce-variation-swatches' ),
			'square'  => esc_html__( 'Square', 'teconce-variation-swatches' ),
		] );
		...
		if ( current_theme_supports( 'woocommerce' ) ) {
			...
			if ( $theme_style ) {
				$options = array_merge( [ 'default' => esc_html__( 'Theme Default', 'teconce-variation-swatches' ) ], $options );
			}
		}

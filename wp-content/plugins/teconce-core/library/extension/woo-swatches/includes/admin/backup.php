...
class Backup {
	 ...
	 * @static
	 *
	 * @var Teconce\VariationSwatches\Admin\Backup
	 */
	 ...
	 * @static
	 *
	 * @return Teconce\VariationSwatches\Admin\Backup An instance of the class.
	 */
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_product_export_product_column_teconce_attributes_type', [ $this, 'add_attributes_type_export_data' ], 10, 2 );
		add_filter( 'woocommerce_product_export_product_column_teconce_variaiton_swatches', [ $this, 'add_variation_swatches_export_data' ], 10, 2 );
	...
	public function restore_attributes_backup_notice() {
		...
		if ( $backup && ! get_option( 'teconce_variation_swatches_ignore_restore' ) ) {
			$backup_date = date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $backup['time'] );
			?>
			<div class="notice notice-warning is-dismissible">
				<p>
					<?php printf( esc_html__( 'Found a backup of your attributes type. This backup was generated at %s.', 'teconce-variation-swatches' ), $backup_date ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'teconce_variation_swatches_restore_backup', '_wpnonce' => wp_create_nonce( 'teconce_variation_swatches_restore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Restore attributes', 'teconce-variation-swatches' ) ?></a> |
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'teconce_variation_swatches_ignore_backup', '_wpnonce' => wp_create_nonce( 'teconce_variation_swatches_ignore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Ignore', 'teconce-variation-swatches' ) ?></a>
				</p>
			</div>
			<?php
		}

		if ( ! $backup && isset( $_GET['message'] ) && 'teconce_variation_swatches_restored_backup' == $_GET['message'] ) {
			?>
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e( 'All attribute types have been restored.', 'teconce-variation-swatches' ) ?></p>
			</div>
			<?php
		}
	}
	...
	public function handle_restore_request() {
		if ( ! isset( $_GET['action'] ) || ! isset( $_GET['_wpnonce'] ) || 'teconce_variation_swatches_restore_backup' != $_GET['action'] ) {
			return;
		}
		...
		if ( $success ) {
			...
			$url = add_query_arg( [ 'message' => 'teconce_variation_swatches_restored_backup' ], $url );
	...
	public function handle_ignore_request() {
		if ( ! isset( $_GET['action'] ) || ! isset( $_GET['_wpnonce'] ) || 'teconce_variation_swatches_ignore_backup' != $_GET['action'] ) {
			return;
		}
	...
	public function restore() {
		...
		delete_transient( 'teconce_variation_swatches_attributes' );
	...
	public function ignore() {
		return update_option( 'teconce_variation_swatches_ignore_restore', time(), false );
	}
	...
	public static function backup() {
		...
		if ( ! empty( $backup['attributes'] ) ) {
			set_transient( 'teconce_variation_swatches_attributes', $backup );
			delete_transient( 'wc_attribute_taxonomies' );
	...
	protected function get_backup_attributes() {
		$backup = get_transient( 'teconce_variation_swatches_attributes' );
	...
	public function add_import_export_columns( $columns ) {
		$columns['teconce_variaiton_swatches'] = esc_html__( 'Variation Swatches', 'teconce-variation-swatches' );
		$columns['teconce_attributes_type']    = esc_html__( 'Attributes Swatches', 'teconce-variation-swatches' );
	...
	public function add_column_to_mapping_screen( $columns ) {
		$columns[ esc_html__( 'Variation Swatches', 'teconce-variation-swatches' ) ]  = 'teconce_variaiton_swatches';
		$columns[ esc_html__( 'Attributes Swatches', 'teconce-variation-swatches' ) ] = 'teconce_attributes_type';
	...
	public function process_import( $product, $data ) {
		if ( empty( $data['teconce_variaiton_swatches'] ) ) {
			return $product;
		}

		$swatches_meta = (array) json_decode( $data['teconce_variaiton_swatches'], true );
		...
		$product->update_meta_data( 'teconce_variaiton_swatches', $swatches_meta );
	...
	public function import_attributes_type( $product, $data ) {
		if ( empty( $data['teconce_attribute_type'] ) ) {
			return;
		}

		$raw_data             = (array) json_decode( $data['teconce_attribute_type'], true );
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				...
				throw new \Exception( sprintf( esc_html__( 'Not able to attach "%s".', 'teconce-variation-swatches' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			/* translators: %s: image URL */
			throw new \Exception( sprintf( esc_html__( 'Unable to use image "%s".', 'teconce-variation-swatches' ), $url ), 400 );
		}

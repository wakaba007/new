<?php if ( $query->have_posts() ) : ?>
    <?php while ( $query->have_posts() ): $query->the_post();
        ...
        <div class="ajax-posts__post col">
            ...
            } elseif ($fpstyle=="eleven"){

                get_template_part('inc/vendor/woocommerce/product-style/content-style-eleven');
    ...
    <?php if (!$this->is_last_page($query)) : ?>
        <div class="ajax-posts__load-more">
            <button class="js-load-more" data-page="<?= $this->get_page_number($query) + 1; ?>">
                <?php _e('Load more', 'ttc-ajax-filter') ?> <i class="teconce-icon-reload"></i>
            </button>
        </div>
    <?php endif; ?>
<?php else: ?>

...
class TokenEntity
{
     ...
     * @param string|array<string,string> $token        The json token string or array containing access_token, refresh_token, expires_in, token_type, scope
     */
    ...
    public function refresh($force = false)
    {
        if ($force || $this->isAboutToExpire()) {
            try {
                ...
                DUP_PRO_Log::trace("Could not refresh token: " . $e->getMessage());
                return false;
            }
        }
    ...
    public function updateProperties($data)
    {
        ...
        $this->expires_in    = isset($data['expires_in']) ? $data['expires_in'] : $this->expires_in;

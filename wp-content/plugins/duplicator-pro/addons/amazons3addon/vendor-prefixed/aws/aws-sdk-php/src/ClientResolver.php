...
use VendorDuplicator\Aws\Api\Validator;
use VendorDuplicator\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use VendorDuplicator\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use VendorDuplicator\Aws\ClientSideMonitoring\Configuration;
use VendorDuplicator\Aws\Configuration\ConfigurationResolver;
use VendorDuplicator\Aws\Credentials\CredentialProvider;
use VendorDuplicator\Aws\Credentials\Credentials;
use VendorDuplicator\Aws\Credentials\CredentialsInterface;
use VendorDuplicator\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use VendorDuplicator\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use VendorDuplicator\Aws\Endpoint\EndpointProvider;
use VendorDuplicator\Aws\Endpoint\PartitionEndpointProvider;
use VendorDuplicator\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use VendorDuplicator\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use VendorDuplicator\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use VendorDuplicator\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use VendorDuplicator\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use VendorDuplicator\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use VendorDuplicator\Aws\EndpointDiscovery\ConfigurationInterface;
use VendorDuplicator\Aws\EndpointDiscovery\ConfigurationProvider;
use VendorDuplicator\Aws\EndpointV2\EndpointDefinitionProvider;
use VendorDuplicator\Aws\Exception\AwsException;
use VendorDuplicator\Aws\Exception\InvalidRegionException;
use VendorDuplicator\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use VendorDuplicator\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use VendorDuplicator\Aws\Signature\SignatureProvider;
use VendorDuplicator\Aws\Token\Token;
use VendorDuplicator\Aws\Token\TokenInterface;
use VendorDuplicator\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \VendorDuplicator\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \VendorDuplicator\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \VendorDuplicator\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \VendorDuplicator\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \VendorDuplicator\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _apply_user_agent($inputUserAgent, array &$args, HandlerList $list)
    {
        ...
        if (isset($args['endpoint_discovery'])) {
            if ($args['endpoint_discovery'] instanceof \VendorDuplicator\Aws\EndpointDiscovery\Configuration && $args['endpoint_discovery']->isEnabled()) {
        ...
        if (isset($args['retries'])) {
            if ($args['retries'] instanceof \VendorDuplicator\Aws\Retry\Configuration) {
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \VendorDuplicator\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \VendorDuplicator\Aws\manifest();

...
namespace VendorDuplicator\Dropbox\GuzzleHttp\Handler;
...
use VendorDuplicator\Dropbox\GuzzleHttp\Promise\FulfilledPromise;
use VendorDuplicator\Dropbox\GuzzleHttp\Psr7;
use VendorDuplicator\Dropbox\GuzzleHttp\Psr7\LazyOpenStream;
use VendorDuplicator\Dropbox\GuzzleHttp\TransferStats;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private $maxHandles;
    /**
     * @param $maxHandles Maximum number of idle handles.
     */
     ...
     *
     * @return \VendorDuplicator\Dropbox\GuzzleHttp\Promise\PromiseInterface
     */
    ...
    private static function createRejection(EasyHandle $easy, array $ctx)
    {
        ...
        if ($easy->onHeadersException) {
            return \VendorDuplicator\Dropbox\GuzzleHttp\Promise\rejection_for(new RequestException('An error was encountered during the on_headers event', $easy->request, $easy->response, $easy->onHeadersException, $ctx));
        }
        ...
        return \VendorDuplicator\Dropbox\GuzzleHttp\Promise\rejection_for($error);
    }
     ...
     *
     * @param $name    Case-insensitive header to remove
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf)
    {
        ...
        if (isset($options['sink'])) {
            $sink = $options['sink'];
            if (!\is_string($sink)) {
                $sink = \VendorDuplicator\Dropbox\GuzzleHttp\Psr7\stream_for($sink);
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) || !\VendorDuplicator\Dropbox\GuzzleHttp\is_host_in_noproxy($host, $options['proxy']['no'])) {
        ...
        if (!empty($options['debug'])) {
            $conf[\CURLOPT_STDERR] = \VendorDuplicator\Dropbox\GuzzleHttp\debug_resource($options['debug']);
